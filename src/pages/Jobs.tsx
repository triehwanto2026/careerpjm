      ...
      <section className="hero-gradient py-16">
        <div className="container">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center max-w-2xl mx-auto mb-8">
            <h1 className="text-3xl md:text-4xl font-bold text-background dark:text-foreground mb-3">Semua Lowongan</h1>
            <p className="text-background/70 dark:text-muted-foreground">Jelajahi {jobs.length} lowongan pekerjaan yang tersedia</p>
          </motion.div>
          <div className="max-w-3xl mx-auto">
            <div className="flex gap-3 p-2 rounded-xl bg-background/10 dark:bg-card/50 backdrop-blur-lg border border-background/20 dark:border-border">
            ...
            {filtered.map((job, i) => (
              <motion.div key={job.id} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}>
                <Link to={`/jobs/${job.id}`} className="block card-elevated p-6 h-full group">
                  ...
                  <p className="text-sm text-muted-foreground mb-2">{(job as any).department || (job as any).category}</p>
