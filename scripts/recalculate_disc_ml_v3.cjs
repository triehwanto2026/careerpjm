...
async function recalculateDiscML() {
  ...
  const discInstrument = instruments?.find(inst => 
    inst.name && inst.name.toUpperCase().includes('DISC')
  );

  if (!discInstrument) {
    console.log('No DISC instrument found.');
    console.log('Available:', instruments?.map(i => i.name).join(', '));
    return;
  }

  console.log(`Found DISC instrument: ${discInstrument.name} (${discInstrument.id})`);
  
  // Process all results that match DISC
