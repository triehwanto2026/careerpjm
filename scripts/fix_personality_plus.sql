-- Fix Personality Plus (4 Temperamen) with standard Florence Littauer word list
-- Each question should have DIFFERENT words, not the same 4 words repeated
...
DECLARE
    ...
    q_id UUID;
BEGIN
    -- Get Personality Plus instrument ID
    SELECT id INTO personality_instrument_id
    FROM test_instruments
    WHERE name = 'Personality Plus (4 Temperamen)'
    LIMIT 1;

    IF personality_instrument_id IS NULL THEN
        RAISE NOTICE 'Personality Plus (4 Temperamen) not found';
        RETURN;
    END IF;

    RAISE NOTICE 'Found Personality Plus instrument: %', personality_instrument_id;

    -- Delete existing questions and options
    DELETE FROM test_question_options 
    WHERE question_id IN (SELECT id FROM test_questions WHERE instrument_id = personality_instrument_id);
    
    DELETE FROM test_questions WHERE instrument_id = personality_instrument_id;
    
    RAISE NOTICE 'Deleted existing Personality Plus questions';

    -- Questions 1-40: Standard Florence Littauer Personality Plus word list with randomized positions
    -- Each row has 4 DIFFERENT words representing the 4 temperaments
    ...
    RAISE NOTICE '✅ Personality Plus (4 Temperamen) fixed with standard Florence Littauer word list - each question has DIFFERENT words';
END $$;
